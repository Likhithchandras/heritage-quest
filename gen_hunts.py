...
def cp(cid, seq, landmark, lat, lng, radius, story, clue, task, riddle, ans, aliases, h1, h2, fact, quiz, opts, correct, pts=50, xp=100, img=None):
    return {
        'id': cid, 'sequence': seq, 'landmark': landmark,
        ...
        'story': story, 'clue': clue, 'observationTask': task,
        'riddle': riddle, 'riddleAnswer': ans,
        'acceptableAnswers': [a.lower().strip() for a in ([ans] + aliases)],
        'hint1': h1, 'hint2': h2, 'learnFact': fact,
        'quizQuestion': quiz, 'quizOptions': opts, 'correctOptionIndex': correct,
        'points': pts, 'xp': xp
    }
